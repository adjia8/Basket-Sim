...
const QUESTION_BANK: Record<QuestionCategory, string[]> = {
  results: [
...
function buildOptions(): PressOption[] {
  return TONES.map((tone) => ({ id: tone, label: TONE_LABELS[tone], tone }));
}
...
export function generateQuestions(count: number): GeneratedQuestion[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    questions.push({ prompt, category, options: buildOptions() });
  }
  return questions;
}
